...
for filename in os.listdir(r"C:\Users\rchen\Downloads\SERP\apartmentguideSERParchive20170608"):
    ...
    if idx >= 23137:
        ...
        if name0<> None:
                sql ='insert into dbo.scraperRight(idx,sitename,rating,ReviewCount) values ('+str(idx)+',\''+name0.encode("utf-8")+'\',\''+value0.encode("utf-8").replace("'","''")+'\',\''+review0.encode("utf-8").replace("'","''")+'\');'
                print sql
                session.execute(sql)
                con.commit()
        ...
        if name1<> None:
                sql ='insert into dbo.scraperRight(idx,sitename,rating,ReviewCount) values ('+str(idx)+',\''+name1.encode("utf-8")+'\',\''+value1.encode("utf-8").replace("'","''")+'\',\''+review1.encode("utf-8").replace("'","''")+'\');'
                print sql
                session.execute(sql)
                con.commit()
        ...
        if name2<> None:
                sql ='insert into dbo.scraperRight(idx,sitename,rating,ReviewCount) values ('+str(idx)+',\''+name2.encode("utf-8")+'\',\''+value2.encode("utf-8").replace("'","''")+'\',\''+review2.encode("utf-8").replace("'","''")+'\');'
                print sql
                session.execute(sql)
                con.commit()
        ...
        #            print header
                    ...
                    sql = 'insert into dbo.scraperLeft(idx,LinkLable,ReviewText) values ('+str(idx)+',\''+navigation.encode('utf-8')+'\',\''+rating.encode('utf-8')+'\');'
                    print sql
                    session.execute(sql)
                    con.commit()                
                    print '\n'
